...
require(["checkUtil", "loglevel", "SymbolNameGenerator", "ElementQuery", "ElementSelect",
        "LayerOperation", "ColorTransformDefinitions", "FilterDefinitions"],
    function(checkUtil, log, sng, eq, es, lo, ctd, fd) {
        ...
        function addLayerInner(timeline) {
            ...
            // 水波纹 素材需要替换为合适的素材，这里暂时使用原来的元件替代
            doc.clipPaste(true);
        ...
        function EditInnerWater() {
            ...
            // var layers = timeline.layers; //图层
            // var waterLayer = layers[INNER_WATER_LAYER_INDEX];
            // var elementLayer = layers[INNER_ElEMENT_LAYER_INDEX];
            //
            //
            // waterLayer.setRigParentAtFrame(elementLayer, 0);

            setParentLayer(timeline, INNER_ElEMENT_LAYER_INDEX, INNER_WATER_LAYER_INDEX, "mask");
            doc.exitEditMode();
        }
        ...
        function Main() {
            ...
            alert("如果需要更好的效果，请把水波纹 换成 更加合适的素材。 ");
        }
