...
// @formatter:on
require(["checkUtil", "loglevel", "xmlPanelUtil", "ElementChecker", "ElementQuery",
    "numpy"],
    function(checkUtil, log, xmlPanelUtil, ec, eq, np) {
        ...
        function checkMotionFrameCount(expression,motionFrameCount) {
            const { frameCount: EXPRESSION_FRAME_COUNT, duration: EXPRESSION_DURATION } = getElementInfo(expression);
            ...
            if (!isMultiple(EXPRESSION_DURATION, motionFrameCount)) {
                console.trace("优化建议：万能表情中单个表情持续了" + EXPRESSION_DURATION + "帧，但输入的表情帧数为" + motionFrameCount + "帧，两者不是倍数关系，建议检查...");
            }
        }
