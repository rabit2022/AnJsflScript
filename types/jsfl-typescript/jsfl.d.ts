...
interface FlashDocument {
    ...
    /** returns a string that specifies, in XML format, the specified profile. If you don抰 pass a value for profileName, the current profile is exported. */
...
interface FlashTimeline {
    ...
    /** Sets the frame.tweenType property to motion for each selected keyframe on the current layer, and converts each frame抯 contents to a single symbol instance if necessary. */
    ...
    /** Retrieves the specified property抯 value for the selected frames. */
    ...
    /** Retrieves the specified property抯 value for the selected layers. */
